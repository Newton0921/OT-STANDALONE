#!/bin/bash
# DNP3 Sequence Anomaly Lab - Blue Setup Script
# OS: Ubuntu 22.04 LTS

echo "[*] Deploying post-exercise forensic environment..."
...
# Make outstation executable and run it in background
chmod +x /opt/dnp3_lab/outstation_sim.py
nohup python3 /opt/dnp3_lab/outstation_sim.py > /opt/dnp3_lab/logs/outstation.out 2>&1 &
sleep 1

# Start tcpdump on port 20000
nohup tcpdump -i any -n -w /opt/dnp3_lab/pcap/dnp3_traffic.pcap tcp port 20000 > /dev/null 2>&1 &
TCPDUMP_PID=$!
sleep 1

# Generate synthetic traffic using python inline to seed PCAP and logs
echo "[*] Seeding normal baseline and anomaly traffic into PCAP and logs..."
python3 -c "
import socket
import time
import struct
...
def send_sbo(src):
    ...
    select_frame = make_dnp3_frame(0xc4, 10, src, select_pld)
    s.send(select_frame)
    s.recv(1024)
    ...
    operate_frame = make_dnp3_frame(0xc4, 10, src, operate_pld)
    s.send(operate_frame)
    s.recv(1024)
    s.close()

def send_direct_operate(src):
    s = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
    s.connect(('127.0.0.1', 20000))
    direct_pld = b'\xc0\xc2\x03\x0c\x01\x28\x01\x07\x00\x01\x01\x01\x00\x00\x00\x00'
    direct_frame = make_dnp3_frame(0xc4, 10, src, direct_pld)
    s.send(direct_frame)
    s.recv(1024)
    s.close()
...
# Anomaly: Direct Operate from Master 66 (Unlisted)
send_direct_operate(66)
time.sleep(2)
...
# Stop tcpdump to save PCAP file
kill $TCPDUMP_PID || true
sleep 1

# Change ownership/permissions so files are readable by blue team participants
chmod -R 777 /opt/dnp3_lab

echo "[*] Forensic environment set up successfully. Outstation remains active on TCP 20000."
#!/bin/bash
# setup.sh - Blue Module
# Generates the post-attack environment for forensics (all in pure Bash).
# Configures the broker, pre-seeds logs, and starts the active/compromised simulation.
...
if [ "$SKIP_INSTALL" = false ]; then
    echo "[*] Preparing Blue Team Forensics Environment..."
...
echo "[*] Generating synthetic log data to simulate past attack perfectly..."
base_time=$(date +%s)
start_time=$((base_time - 3600)) # 1 hour ago
...
# 2. Normal telemetry baseline (15 entries)
...
# 3. ROGUE ATTACK
attack_time=$((start_time + 155))
echo "${attack_time}: New client connected from ${ip_rogue} as rogue_solar_client (p2, c1, k60)." >> /tmp/mqtt_lab/log/mosquitto.log
echo "${attack_time}: Client rogue_solar_client PUBLISH (d0, q0, r1, m0, 'grid/solar/site7/kw', ... (6 bytes))" >> /tmp/mqtt_lab/log/mosquitto.log
echo "${attack_time}: 	Payload: 150.00" >> /tmp/mqtt_lab/log/mosquitto.log
echo "${attack_time+1}: Client rogue_solar_client disconnected." >> /tmp/mqtt_lab/log/mosquitto.log

# Ground truth remains normal at attack time
timestamp_attack=$(date -d "@$attack_time" +"%Y-%m-%dT%H:%M:%S" 2>/dev/null || date -r "$attack_time" +"%Y-%m-%dT%H:%M:%S")
echo "${timestamp_attack} - 49.88 kW" >> /tmp/mqtt_lab/log/ground_truth.log
...
# Start the Legitimate Publisher in the background
...
# Start an active rogue loop to keep the dashboard showing the compromised value (Python using paho-mqtt)
cat << 'EOF' > /tmp/mqtt_lab/active_rogue.py
#!/usr/bin/env python3
import time
import paho.mqtt.client as mqtt
...
if __name__ == "__main__":
    main()
EOF
chmod +x /tmp/mqtt_lab/active_rogue.py
nohup python3 /tmp/mqtt_lab/active_rogue.py > /dev/null 2>&1 &
echo $! > /tmp/mqtt_lab/rogue.pid

# Populate ACL file and configs on disk for blue team inspection
cp /tmp/mqtt_lab/config/mosquitto.conf /tmp/mqtt_lab/mosquitto.conf 2>/dev/null || true
cp /tmp/mqtt_lab/config/acl /tmp/mqtt_lab/acl 2>/dev/null || true

# Set permissions
sudo chmod -R 777 /tmp/mqtt_lab

echo "[+] Blue Module Setup Complete! Logs are ready in /tmp/mqtt_lab/log/"